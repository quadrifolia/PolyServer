...
# Generate GDPR/DSGVO compliance documentation if enabled
if [ "${GDPR_ENABLED:-false}" = "true" ]; then
  echo "Generating GDPR/DSGVO compliance documentation..."

  mkdir -p "${OUTPUT_DIR}/dsgvo"

  # Generate DSGVO configuration and documentation files
  for dsgvo_file in "${TEMPLATE_DIR}/dsgvo"/*.template; do
    if [ -f "$dsgvo_file" ]; then
      filename=$(basename "$dsgvo_file" .template)
      render_template "$dsgvo_file" "${OUTPUT_DIR}/dsgvo/$filename"
    fi
  done

  # Copy non-template DSGVO files
  for dsgvo_file in "${TEMPLATE_DIR}/dsgvo"/*.md; do
    if [ -f "$dsgvo_file" ]; then
      filename=$(basename "$dsgvo_file")
      cp "$dsgvo_file" "${OUTPUT_DIR}/dsgvo/$filename"
    fi
  done
fi
